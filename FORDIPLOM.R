...
ggplot(art) + 
  aes(x = Width) + 
  geom_histogram(bins = 30L, fill = "lightpink", color = "black") + 
  theme_minimal() + labs(x = 'Ширина', y='Количество работ', Name = '')

art$Width <- art$Width*art*Depth
art$Width <-  art$Width/10000
...
ggplot(art) + 
  aes(x = LotNum) + 
  geom_histogram(bins = 30L, fill = "lightpink", color = "black" ) + 
  theme_minimal() + labs(x = 'РќРѕРјРµСЂ Р»РѕС‚Р°', y='РљРѕР»РёС‡РµСЃС‚РІРѕ Р°СЂС‚-РѕР±СЉРµРєС‚РѕРІ', Name = "Р Р°СЃРїСЂРµРґРµР»РµРЅРёРµ РїРѕ РЅРѕРјРµСЂСѓ Р»РѕС‚Р°")
...
#Дата
ggplot(data = art, aes(x = Date)) + geom_bar(fill = 'lightpink', color = " black", width=4) +
  labs(x = 'Р”Р°С‚Р°', y='РљРѕР»РёС‡РµСЃС‚РІРѕ С‚СЂР°РЅР•Р°РєС†РёР№')+theme_classic()
...
#СЂР°СЃРїСЂРµРґРµР»РµРЅРёРµ РїРѕ РІСЂРµРјРµРЅРё
ggplot(data = art, aes(x = Date)) + geom_bar(fill = 'lightpink', color = "lightpink", width=4) +
  labs(x = 'Р”Р°С‚Р°', y='РљРѕР»РёС‡РµСЃС‚РІРѕ С‚СЂР°РЅР•Р°РєС†РёР№', Name = "Р Р°СЃРїСЂРµР»РµРЅРёРµ С‚СЂР°РЅР•Р°РєС†РёР№ РїРѕ РІСЂРµРјРµРЅРё")+theme_classic()

#распределение продаж по годам
table(art$Month)
ggplot(data = art, aes(x = Month)) + geom_bar() + theme_classic() +   labs(x = 'РњРµСЃСЏС†', y='РљРѕР»РёС‡РµСЃС‚РІРѕ С‚СЂР°РЅР•Р°РєС†РёР№')
...
ggplot(data) + 
  geom_bar(aes(x=year, y=value),stat="identity", fill="grey",colour="white")+ 
  geom_line(aes(x=year, y=number*10000),stat="identity",color="red")+ 
  labs(title= "Объем рынка и количество продаж", 
       x="Год",y="Рынок американского искусства с 2004") 
...
#сравниваем модель с площадью и без – с площадью (1) лучше

#нужен ли логарифм - нужен
boxCox(mod_1)
#добавляем логарифм
...
#сравниваем по акаике лучше ли с логарифмом - да
AIC(mod_1, mod_2)
#графики не очень - а что с этим делать?
plot(mod_2, which = 1)
plot(mod_2, which = 2)
plot(mod_2, which = 3)

#мультиколлинеарности нет
vif(mod_2) #мультиколлинеарности нет


bptest(mod_2) #нет зависимости ошибок от регрессоров

resettest(mod_2) #отвергается, есть пропущенные степени
...
plot(H, type = "h")
which(H>0.2)
# нет сильных выбросов в данных, выборка достаточно однородная, есть один выброс, тот же что и в расстоянии Кука
...
anova(mod2.1, mod2.2) #лучше модель без дня
...
#создаём инструментальную переменную
...
sales <- subset(art, select = c(- USA, - Name, - Dead, - Born, - Died, - Depth, - Square, - Medium, - Day, - Date))
sales <-  dplyr::filter(sales, AucPrice>62)
sales <-  dplyr::filter(sales, AucPrice<250000)
sales$Month <- as.factor(sales$Month)
sales$Year <- as.factor(sales$Year)
...
mod_fineart <- lm(I(log(AucPrice)) ~ . - Type + I(Height*Width) - Height - Width -EstMin + log(EstMin) - EstMax + log(EstMax), data = fineart)
summary(mod_fineart)

uniquegraphics <-  dplyr::filter(sales, Type == 2)
mod_uniquegraphics <- lm(I(log(AucPrice)) ~ . - Type + I(Height*Width) - Height - Width -EstMin+log(EstMin)-EstMax+log(EstMax), data = uniquegraphics)
summary(mod_uniquegraphics)

tiragegraphics <-  dplyr::filter(sales, Type == 3)
mod_tiragegraphics <- lm(I(log(AucPrice)) ~ . - Type + I(Height*Width) - Height - Width -EstMin+log(EstMin)-EstMax+log(EstMax), data = tiragegraphics)
summary(mod_tiragegraphics)

sculpture <-  dplyr::filter(sales, Type == 4)
mod_sculpture <- lm(I(log(AucPrice)) ~ . - Type + I(Height*Width) - Height - Width -EstMin+log(EstMin)-EstMax+log(EstMax), data = sculpture)
summary(mod_sculpture)

photograph <-  dplyr::filter(sales, Type == 5)
mod_photograph <- lm(I(log(AucPrice)) ~ . - Type + I(Height*Width) - Height - Width -EstMin+log(EstMin)-EstMax+log(EstMax), data = photograph)
...
masterpiece <-  dplyr::filter(masterpiece, AucPrice>250000)
...
mod_online2 <- lm(I(log(AucPrice)) ~ . - Square, data = online)

AIC(mod_online, mod_online2) #первая лучше
...
#выгружаем
stargazer(mod_online,
          title="Гедонистическая регрессия для онлайн-аукционов", header = FALSE, type="html", out = "ОнлайнАукционы.html",
...
###heck$AucPrice[heck$AucPrice == 0] <- NA
...
heckit <- selection(selection = Sold ~ Ages + Signed + Stamped + Inscribed + Dating + LotNum  + Year + SothChr + Currency + Premium + Type + I(Height * Width) + EstMin + EstMax,
                    outcome = I(log(AucPrice)) ~ Ages + Signed + Stamped + Inscribed + Dating + LotNum  + Year + SothChr + Currency + Premium + Type + I(Height * Width) + log(EstMin) + log(EstMax),
                    data = hk,
                    method = "2step" ) 
summary(heckit)
...
art <-  dplyr::filter(art, AucPrice<342889)
...
#РїРѕСЃС‚СЂРѕРёРј РјРѕРґРµР»СЊ
mod_pca_1 <- PCA(clust)

#СЃРѕР±СЃС‚РІРµРЅРЅС‹Рµ С‡РёСЃР»Р° - СЃСѓРјРјР° СЃРѕР±СЃС‚РІРµРЅРЅС‹С… С‡РёСЃРµР» - СЃР»РµРґ РјР°С‚СЂРёС†С‹
mod_pca_1$eig

#РіСЂР°С„РёРє РєР°РјРµРЅРёСЃС‚РѕР№ РѕСЃС‹РїРё - СЃРєРѕР»СЊРєРѕ РїСЂРѕС†РµРЅС‚РѕРІ РґРёСЃРїРµСЂСЃРёРё РѕР±СЉСЏСЃРЅСЏРµС‚СЃСЏ РєРѕРјРїРѕРЅРµРЅС‚РѕР№
...
#РјР°С‚СЂРёС†Р° РЅР°РіСЂСѓР•РѕРє
mod_pca_1$var$coord
corrplot(mod_pca_1$var$coord, is.corr = FALSE) 
corrplot(mod_pca_1$var$coord, method = "number", is.corr = FALSE) + theme_minimal()

#РєРІР°РґСЂР°С‚С‹ РєРѕСЌС„С„РёС†РёРµРЅС‚РѕРІ
mod_pca_1$var$cos2
rowSums(mod_pca_1$var$cos2)
corrplot(mod_pca_1$var$cos2, is.corr = FALSE)


#РєР°РєРёРµ РІРµРєС‚РѕСЂС‹ РѕР±СЉСЏСЃРЅСЏСЋС‚?
fviz_cos2(mod_pca_1, choice = "var", axes = 1:2, Name = "РћР±СЉСЏСЃРЅСЏСЋС‰РёРµ РІРµРєС‚РѕСЂС‹",  fill = "lightpink", color = "black") 
...
fviz_dend(model_clust, palette = "simpsons", rect = TRUE, rect_fill = TRUE, cex = 0.8,
...
#выделяет три кластера
#Строим кластеры в пространстве главных компонент
